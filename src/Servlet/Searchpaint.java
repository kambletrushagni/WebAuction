...
public class Searchpaint extends HttpServlet
{
	//Connection con;
/*	public void init(ServletConfig sc) throws ServletException
	...
	try
	{
		...
		System.out.println("categories>>>"+cstring+"string>>"+sstring);
		...
		pw.println("<p align=right><FORM name=form1 action='./ItemDetails'  method=post >Search:<INPUT name=sstring> ");
		...
		if((!cstring.equals("Choose Category")))
		{
			System.out.println("in second");
			results = searchCatInfo("Electronics");
			val=2;
		}
		 //if( (sstring.length()==0) && (cstring.equals("Choose Category")))
		if(cstring.equals("Choose Category"))
		 ...
		 /*if( (sstring.length() != 0) && (!cstring.equals("Choose Category")))
		{
			...
			results =searchCatItemInfo(cstring);
			System.out.println("searchCatItemInfo(cstring)");
			val=4;
		}*/
	/*if((sstring.length()!=0) && (cstring.equals("Choose Category")))
			if(cstring.equals("Choose Category"))
	...
	catch(Exception e){e.printStackTrace();System.out.println("t2ry");}
	} 
	
	
	public Vector searchCatInfo(String s)throws RemoteException,Exception{
...
//	 Statement stmt1=con.createStatement();
	...
	//ps.setString(1,s);
        	 String str="select catid from category_master where cat_name='Electronics'";
...
//	ps.close();
	   if(id != null)
	   {
		...
		//psmt.setString(1,id);	
		   String str2="select * from item_master where catid ="+id+"";
		ResultSet recs = stmt.executeQuery(str2);
		while(recs.next())
		{
	...
	}catch(Exception e){e.printStackTrace();System.out.println("333");}
	return data;
	}
     
    public Vector searchCatItemInfo(String cName)throws RemoteException,Exception{
	Vector data = new Vector();
	Class.forName("com.mysql.jdbc.Driver");
    Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/demo", "tashu", "123456");
	...
	String str="select catid from category_master where cat_name="+cName+"";
		System.out.println("cname>>"+cName+"Query>>>"+str);
